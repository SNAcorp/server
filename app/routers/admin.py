...
async def change_user_role(request: Request, user_id: int,
    ...
    updated_user = await update_user_role(user, data["role"])
    return updated_user
...
async def update_user_details(user_id: int,
                              ...
                              current_user: User = Depends(get_admin_user)):
    user = await check_user(user_id, current_user=current_user)
    updated_user = await update_user(user, user_data)
...
async def block_user_route(user_id: int,
    ...
    blocked_user = await block_user(user)
    return blocked_user
...
async def unblock_user_route(user_id: int,
                             current_user: User = Depends(get_admin_user)):
    user = await check_user_for_superuser(user_id, current_user=current_user)
    unblocked_user = await unblock_user(user)
